% Generate random sets of parameters to evaluate equilibrium of Model 3A and
% 3B
...
results3APos = zeros(1,5);
results3BPos = zeros(1,4);

coexist3A = 0; % Number of all positive equilibria for 3A
...
for j = 1:reps
    ...
    if (length(equiV) == sum(equiV > 0))
        coexist3A = coexist3A + 1;
    end
    
    % Evaluate 3B equilibria
    equiNV = zeros(1,4);
    equiNV = Model3BEqui(p);
    
    % Store results and check for coexistence
    for i = 1:length(equiNV(:,1))
        
        comp3B = comp3B + 1;
        results3B(comp3B,:) = equiNV(i,:);
        
        if (length(equiNV(i,:)) == sum(equiNV(i,:) > 0))
            ...
            if (length(equiV) == sum(equiV > 0))
                ...
                results3APos(comp,:) = equiV;
                results3BPos(comp,:) = equiNV(i,:);
                
                % Put back equilibrium values into equations
                
                verif3A(comp,:) = Model3A_Verif(p,equiV);
                verif3B(comp,:) = Model3B_Verif(p,equiNV(i,:));
                ...
                % DOM recycling
                eco3A(comp,1) = p(15)*p(9)*p(5)*equiV(1)*equiV(2) + (p(9)-p(17)*p(18))*p(6)*equiV(1)*equiV(5);
                eco3B(comp,1) = p(15)*p(9)*p(5)*equiNV(i,1)*equiNV(i,2);
                
                % Export
                eco3A(comp,2) = p(10)*p(12)*equiV(2)^2;
                eco3B(comp,2) = p(10)*p(12)*equiNV(i,2)^2;
                
                % Carbon sink
                eco3A(comp,3) = p(33)*eco3A(comp,2)*p(35) + (p(9)*p(37)-p(17)*p(40)*p(18))*p(6)*equiV(1)*equiV(5)*p(38);
                eco3B(comp,3) = p(33)*eco3B(comp,2)*p(35);
                
                % Productivity
                eco3A(comp,4) = p(9)*p(1)*equiV(1)*equiV(3)/(equiV(3) + p(2));
                eco3B(comp,4) = p(9)*p(1)*equiNV(1)*equiNV(3)/(equiNV(3) + p(2));
            end
...
meanPar = mean(parameters);
sdCorrPar = std(parameters)./meanPar;
...
% Go to log-space
logResults3A = log10(results3APos(:,1:4));
logResults3B = log10(results3BPos);

logEco3A = log10(eco3A);
logEco3B = log10(eco3B);


for i = 1:length(logResults3A(1,:))
    
    %Check for normality
    [nonNormal3A(i),p3A(i)] = kstest(logResults3A(:,i));
    [nonNormal3B(i),p3B(i)] = kstest(logResults3B(:,i));
    
end

% Calculate median
median3A = median(logResults3A);
median3B = median(logResults3B);

medianEco3A = median(logEco3A);
medianEco3B = median(logEco3B);

% Calculate Interquatile range
iqr3A = iqr(logResults3A);
iqr3B = iqr(logResults3B);

iqrEco3A = iqr(logEco3A);
iqrEco3B = iqr(logEco3B);

% Calculate difference between 3A and 3B

diffLogResults3A_3B = logResults3A - logResults3B;

diffLogEco3A_3B = logEco3A - logEco3B;

for i = 1:length(logResults3A(1,:))
    
    %Check for normality
    [diff3A_3BNormal(i),pNormallDiff3A_3B(i)] = kstest(diffLogResults3A_3B(:,i));
    
    %Median different from 0?
    pDiff3A_3B(i) = signrank(diffLogResults3A_3B(:,i));
    
    
end

for j = 1:length(logEco3A(1,:))
    
    % Check for normality
    [diffEco3A_3BNormal(j),pNormalDiffEco3A_3B(j)] = kstest(diffLogEco3A_3B(:,j));

    %Median different from 0?
    pDiffEco3A_3B(j) = signrank(diffLogEco3A_3B(:,j));
    
end

% Find median and IQR
medianDiff3A_3B = median(diffLogResults3A_3B);
iqrDiff3A_3B = iqr(diffLogResults3A_3B);

medianDiffEco3A_3B = median(diffLogEco3A_3B);
iqrDiffeEco3A_3B = iqr(diffLogEco3A_3B);
...
scatter(results3BPos(:,1),results3APos(:,1),sz,'black' )
pbaspect([1 1 1])
plot(z,z,'g^','MarkerSize',7,'LineWidth', 2)
set(gca,'xscale','log')
set(gca,'yscale','log')
...
y = [10^-1, 10^11];
line(x,y,'Color','red','LineWidth',1.5)
scatter(results3BPos(:,2),results3APos(:,2),sz,'black' )
pbaspect([1 1 1])
plot(z,z,'g^','MarkerSize',7,'LineWidth', 2)
set(gca,'xscale','log')
set(gca,'yscale','log')
...
y = [10^-6, 10^2];
line(x,y,'Color','red','LineWidth',1.5)
scatter(results3BPos(:,3),results3APos(:,3),sz,'black' )
pbaspect([1 1 1])
plot(z,z,'g^','MarkerSize',7,'LineWidth', 2)
set(gca,'xscale','log')
set(gca,'yscale','log')
...
y = [10^-6, 10^4];
line(x,y,'Color','red','LineWidth',1.5)
scatter(results3BPos(:,4),results3APos(:,4),sz,'black' )
pbaspect([1 1 1])
plot(z,z,'g^','MarkerSize',7,'LineWidth', 2)
set(gca,'xscale','log')
set(gca,'yscale','log')
...
y = [10^-7, 10^6];
line(x,y,'Color','red','LineWidth',1.5)
scatter(eco3B(:,4),eco3A(:,4),sz,'black')
pbaspect([1 1 1])
plot(z,z,'g^')
set(gca,'xscale','log')
set(gca,'yscale','log')
...
y = [10^-6, 10^4];
line(x,y,'Color','red','LineWidth',1.5)
scatter(eco3B(:,1),eco3A(:,1),sz,'black')
pbaspect([1 1 1])
plot(z,z,'g^','MarkerSize',7,'LineWidth', 2)
set(gca,'xscale','log')
set(gca,'yscale','log')
...
y = [10^-7, 10^4];
line(x,y,'Color','red','LineWidth',1.5)
scatter(eco3B(:,2),eco3A(:,2),sz,'black' )
pbaspect([1 1 1])
plot(z,z,'g^','MarkerSize',7,'LineWidth', 2)
set(gca,'xscale','log')
set(gca,'yscale','log')
...
y = [10^-13, 10^4];
line(x,y,'Color','red','LineWidth',1.5)
scatter(eco3B(:,3),eco3A(:,3),sz,'black' )
pbaspect([1 1 1])
plot(z,z,'g^')
set(gca,'xscale','log')
set(gca,'yscale','log')
