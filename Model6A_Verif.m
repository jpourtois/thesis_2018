...
function xprime = Model6A_Verif(p,equi)
    ...
    xprime2 = rC*C*Pin/(Pin + kInC) - phiC*C*Vc - gammaC*C*Z - lambdaC*C - sigmaC*C;
    xprime3 = pG*Z*(qH*gammaH*H + qC*gammaC*C)/qZ - lambdaZ*Z - gammaZ*Z^2;
    xprime4 = -omega*(Pin - pSub) - qH*rH*H*Pin/(Pin + kInH) - qC*rC*C*Pin/(Pin + kInC) + qZ*lambdaZ*Z + qH*lambdaH*H + qC*lambdaC*C + (1 - pG - pEx)*(qH*gammaH*H*Z + qC*gammaC*C*Z) + psiH*H*Porg + psiC*C*Porg;
    xprime5 = qV*thetaH*Vh + qV*thetaC*Vc + (qH - qV*betaH)*phiH*H*Vh + (qC - qV*betaC)*phiC*C*Vc + qH*sigmaH*H + qC*sigmaC*C + pEx*Z*(qH*gammaH*H + qC*gammaC*C) - psiH*H*Porg - psiC*C*Porg;
