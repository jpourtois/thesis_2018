% Generate random sets of parameters to evaluate equilibrium of Model 7A and
% 4B
...
for (j = 1:reps)
    ...
    %If coexistence for both, store results
    if coexistVLogic
        ...
        for i = 1:length(equiNVMult(:,1))
            ...
            if (sum(equiNV > 0) == length(equiNV))
                ...
                verif7A(comp,:) = Model7A_Verif(p,equiV);
                verif4B(comp,:) = Model4B_Verif(p,equiNV);
                ...
                eigenVal7A = Model7A_Stability(p,equiV);
                
                realEigen7A = double(real(eigenVal7A));
                imagEigen7A = double(imag(eigenVal7A));
                
     
                % Check for real part = 0
                if sum(realEigen7A ~= 0) ~= length(realEigen7A)
                ...
                if sum(realEigen7A < 0) == length(realEigen7A)
                    ...
                    if sum(imagEigen7A == 0) == length(imagEigen7A)
                ...
                eigenVal4B = Model4B_Stability(p,equiNV);
                
                realEigen4B = double(real(eigenVal4B));
                imagEigen4B = double(imag(eigenVal4B));
                
     
                % Check for real part = 0
                if sum(realEigen4B ~= 0) ~= length(realEigen4B)
                ...
                if sum(realEigen4B < 0) == length(realEigen4B)
                    ...
                    if sum(imagEigen4B == 0) == length(imagEigen4B)
...
logResults7A = log10(results7APos(:,1:5));
logResults4B = log10(results4BPos);

logEco7A = log10(eco7A);
logEco4B = log10(eco4B);

for i = 1:length(logResults7A(1,:))
    
    %Check for normality
    [nonNormal7A(i),p7A(i)] = kstest(logResults7A(:,i));
    [nonNormal4B(i),p4B(i)] = kstest(logResults4B(:,i));
    
end

% Calculate median
median7A = median(logResults7A);
median4B = median(logResults4B);

medianEco7A = median(logEco7A);
medianEco4B = median(logEco4B);

% Calculate Interquatile range
iqr7A = iqr(logResults7A);
iqr4B = iqr(logResults4B);

iqrEco7A = iqr(logEco7A);
iqrEco4B = iqr(logEco4B);

% Calculate difference between 7A and 4B

diffLogResults7A_4B = logResults7A - logResults4B;

diffLogEco7A_4B = logEco7A - logEco4B;

for i = 1:length(logResults7A(1,:))
    
    %Check for normality
    [diff7A_4BNormal(i),pNormallDiff7A_4B(i)] = kstest(diffLogResults7A_4B(:,i));
        
    %Median different from 0?
    pDiff7A_4B(i) = signrank(diffLogResults7A_4B(:,i));
    
    
end

for j = 1:length(logEco7A(1,:))
    
    % Check for normality
    [diffEco7A_4BNormal(j),pNormalDiffEco7A_4B(j)] = kstest(diffLogEco7A_4B(:,j));

    %Median different from 0?
    pDiffEco7A_4B(j) = signrank(diffLogEco7A_4B(:,j));
    
end

% Find median and IQR
medianDiff7A_4B = median(diffLogResults7A_4B);
iqrDiff7A_4B = iqr(diffLogResults7A_4B);

medianDiffEco7A_4B = median(diffLogEco7A_4B);
iqrDiffeEco7A_4B = iqr(diffLogEco7A_4B);

% Median viruses

med_vh = median(log10(results7APos(:,7)));
med_vc = median(log10(results7APos(:,6)));

iqr_vh = iqr(log10(results7APos(:,7)));
iqr_vc = iqr(log10(results7APos(:,6)));
